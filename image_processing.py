...
def lghtn(image_surf):
    rows = image_surf.get_size()[0]
    cols = image_surf.get_size()[1]
    pixels3d = pg.surfarray.pixels3d(image_surf)
    for x in range(rows):
        for y in range(cols):
            pixels3d[x,y] = intensify2(intensify(pixels3d[x,y],10))
...
def darkn(image_surf):
    ...
    for x in range(rows):
        for y in range(cols):
            pixels3d[x,y] = dullify2(dullify(pixels3d[x,y],10))
